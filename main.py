...
if os.path.exists(v1_path) and os.path.isdir(v1_path):
    try:
        ...
        print(
            f"Warning: Access Denied. Could not remove legacy directory '{v1_path}'. Please check permissions or if files are in use, then remove manually if needed.")
    except OSError as e:
...
if os.path.exists(v2_path) and os.path.isdir(v2_path):
    try:
        ...
        print(
            f"Warning: Access Denied. Could not remove legacy directory '{v2_path}'. Please check permissions or if files are in use, then remove manually if needed.")
    except OSError as e:
...
if os.path.exists(txt_path) and os.path.isfile(txt_path):
    try:
        ...
        print(
            f"Warning: Access Denied. Could not remove legacy file '{txt_path}'. Please check permissions or if the file is in use, then remove it manually if needed.")
    except OSError as e:
...
try:
    ...
    if original_create_default_https_context is None or \
        ...
        print(
            Fore.YELLOW + "SSL default HTTPS context seems to be already modified. Skipping certifi patch." + Style.RESET_ALL)
except ImportError:
    print(
        Fore.RED + "Certifi library not found. SSL certificate verification might fail until it's installed." + Style.RESET_ALL)
...
if __name__ == "__main__":
        ...
        for dependency in lines:
            ...
            try:
                if debug:
                    ...
                    subprocess.check_call(full_command, timeout=1200, stdout=subprocess.DEVNULL,
    ...
    def safe_remove_file(file_path):
        ...
        if os.path.exists(file_path) and os.path.isfile(file_path):
            try:
                ...
                print(
                    Fore.YELLOW + f"Warning: Access Denied. Could not remove '{file_path}'. Check permissions or if file is in use." + Style.RESET_ALL)
            except OSError as e:
            ...
            if current_version != latest_tag:
                ...
                if not is_container():
                    if "--autoupdate" in sys.argv:
                        ...
                        print(
                            "Note: If your terminal is not interactive, you can use the --autoupdate argument to skip this prompt.")
                        ...
                        if os.path.exists(db_bak_path) and os.path.isdir(db_bak_path):
                            try:
                                shutil.rmtree(db_bak_path)
                                    ...
                                    OSError) as e:  # Create a timestamped backup to avoid upgrading without first having a backup
                                db_bak_path = f"db.bak_{int(datetime.now().timestamp())}"
                                print(
                                    Fore.YELLOW + f"WARNING: Couldn't remove db.bak folder: {e}. Making backup with timestamp instead." + Style.RESET_ALL)
                    ...
                    if download_resp.status_code == 200:
                        ...
                        if os.path.exists("update") and os.path.isdir("update"):
                            try:
                                ...
                                print(
                                    Fore.RED + f"WARNING: Could not remove previous update directory: {e}" + Style.RESET_ALL)
                                return
                        ...
                        if os.path.exists("update/main.py"):
                            ...
                            try:
                                ...
                                try:  # If backup fails, just remove the current file
                                    ...
                                    print(
                                        Fore.RED + "Warning: Could not backup or remove current main.py" + Style.RESET_ALL)
                            ...
                            for file in files:
                                ...
                                if os.path.exists(dst_path):
                                    ...
                                    try:
                                        ...
                                        print(
                                            Fore.YELLOW + f"Could not create backup of {dst_path}: {e}" + Style.RESET_ALL)
                        ...
                        try:
                            ...
                            print(
                                Fore.RED + f"WARNING: update folder could not be removed: {e}. You may want to remove it manually." + Style.RESET_ALL)
                        ...
                        print(
                            Fore.RED + "Failed to download the update. HTTP status: {download_resp.status_code}" + Style.RESET_ALL)
                        return
        else:
            print(
                Fore.RED + f"Failed to fetch latest release info. HTTP status: {latest_release_resp.status_code}" + Style.RESET_ALL)
        ...
        try:
            ...
            if "OCR object created successfully" in result.stdout:
                ...
                if "DLL" in result.stderr:
                    ...
                    print(
                        Fore.YELLOW + "Please install the latest Visual C++ Redistributable from Microsoft:" + Style.RESET_ALL)
                    print(Fore.YELLOW + "https://aka.ms/vs/17/release/vc_redist.x64.exe" + Style.RESET_ALL)
                else:
                    print(
                        Fore.YELLOW + "OCR initialization failed. This might affect CAPTCHA solving." + Style.RESET_ALL)
                return False
        except Exception as e:
    ...
    async def load_cogs():
        cogs = ["olddb", "control", "alliance", "alliance_member_operations", "bot_operations", "logsystem",
                "support_operations", "gift_operations", "changes", "w", "wel", "other_features", "bear_trap",
                "id_channel", "backup_operations", "bear_trap_editor"]
